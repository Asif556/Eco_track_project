...
const DiseaseDisplay = ({ uploadedImage, onReset }) => {
    ...
    const handleImageUpload = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/predict_plant_disease`, formData, {
            ...
            if (response.data.error) {
                ...
                if (confidence < 90) {
                    setPrediction({
                        diseaseId: classIndex,
                        diseaseName: "Unknown Disease",
                        confidence: confidence,
                        isHealthy: false,
                        fertilizer: "Consult an expert",
                        pesticide: "Consult an expert",
                        description: "The system couldn't confidently identify this disease. Please consult with a plant disease specialist for accurate diagnosis.",
                        preventiveMeasures: [
                            "Take multiple clear photos from different angles",
                            "Consult with a local agricultural extension service",
                            "Compare with known disease symptoms in your area",
                            "Isolate the plant to prevent potential spread"
                        ]
                    });
                } else {
                    ...
                    {loading ? (
                        ...
                        <div className={`h-full rounded-xl shadow-lg p-6 ${
                            ...
                            <h2 className={`text-3xl font-bold mb-6 ${
                                ...
                                {prediction.isHealthy ? 'Healthy Plant' : prediction.diseaseName === 'Unknown Disease' ? 'Uncertain Diagnosis' : 'Disease Detected'}
                            </h2>
                            
                            <div className="space-y-6">
                                <div className="p-6 rounded-lg bg-white overflow-y-auto max-h-40">
                                    <h3 className="font-semibold text-gray-600 mb-2">Diagnosis Information</h3>
                                ...
                                <div className="p-6 rounded-lg bg-white">
                                    <h3 className="font-semibold text-gray-600 mb-2">
                                        {prediction.diseaseName === 'Unknown Disease' ? 'Recommended Actions' : 'Preventive Measures'}
                                    </h3>
                        ...
                        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-xl shadow-lg p-6">
                            ...
                            <button 
                                ...
                                className="mt-4 bg-[#082B13] text-white px-4 py-2 rounded-lg hover:bg-[#0a3a1a] transition-all duration-300"
                            >
                                Try Again
                            </button>
                        </div>
